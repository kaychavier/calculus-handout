...
\begin{exe}Fixe uma saída K, onde \(x^2 + y^2 = K\). As curvas de nível de \(f\) são infinitos círculos de diferentes raios.

\[f(x, y) = x^2 + y^2\]

Neste caso, fixando a saída em 1, temos:
...
{x^2 + y^2};
\addplot3 [only marks, mark=*] coordinates {(0, 0, 1)};
\end{axis}
\end{tikzpicture}
\end{center}

Plotando a curva de nível da função \(f(x, y) = x^2 + y^2\) no ponto z = 1:
